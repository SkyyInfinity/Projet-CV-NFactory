...
if ( ! function_exists( 'lifepropulse_setup' ) ) :
	...
	function lifepropulse_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'lifepropulse' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'lifepropulse_custom_background_args',
				array(
...
function lifepropulse_scripts() {
	...
	wp_style_add_data( 'lifepropulse-style', 'rtl', 'replace' );

	wp_enqueue_script( 'lifepropulse-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
